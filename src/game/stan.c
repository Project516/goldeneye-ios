...
// D253: the three middle params are f32 on the N64 (o32 passes them in GPRs
// either way, and the body never uses them), but a host ABI that splits float
// and integer argument registers places the 6th arg (outFlags) differently
...
// match it or outFlags lands in the wrong register: a stale register read and
// a SEGV on the FORCECROUCH/LADDER store. From upstream 1db6992e.

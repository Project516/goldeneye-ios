...
> below: the wallet-Bond photo's "180-degree-rotated / mis-transformed"
> description was made from an upside-down capture, so at least the vertical
> component is the capture bug, not the model transform. Re-judge RC1 from a
> fresh right-way-up capture before doing any more work on it. Writer fixed
...
|---|---|---|---|
| **RC2**, mip-chain contamination (LOD rows uploaded as image rows) | open | Fixed. `import_texture()` clips a full-width LOD block to the SETTILESIZE base height; GL builds correct mips. Knob `Video.FixMipTextures` (default on; `=0` gives the old byte-identical-to-golden behaviour). | `port/fast3d/gfx_pc.cpp`; §F RC2 |
| **RC4**, palette "off-by-one" in `palette_to_rgba32` | proposed 1-line fix | Retracted. The current decode is correct RGBA5551; the "spec" line in §2 was ARGB1555. Applying the "fix" rotates every channel and moves alpha. Do not touch. | `gfx_pc.cpp:~835`; §2 RC4, §F |
...
| **D161**, Depot (`-level_30`) ceiling blue speckle + radial rays (was §3 "blue ground" / B2) | attributed to RC2+RC3+RC4+fog | Fixed. A 16x16 CI8 tile drawn with `G_TT_NONE` (TLUT disabled) was palette-looked-up against a stale `rdp.palette`. fast3d now routes CI+`G_TT_NONE` to I (intensity). Not RC2/RC3/RC4/filtering. | `port/fast3d/gfx_pc.cpp import_texture()`; §F D161, porting-notes.md §D |
| **RC1**, wallet-Bond photo (mode-select / folder screen) garble | "implement G_SETTEX" | Partially open. The interlace comb is gone (D159). The photo now renders as a recognisable grayscale portrait but is still 180-degree-rotated / mis-transformed, which is the D75 front-end-model family (model-GDL transform, not a fast3d opcode gap; `texLoadFromGdl` already expands G_SETTEX on PC, see §6b). Parked, cosmetic, below crash work. | §6b; GRAPHICS-BACKLOG D149; §F D75/D80/D82/D83 |
| **RC3**, wrap-period mismatch (non-PoT dims "repeat oddly") | open | In progress. The dead D74 pre-wrap block is now opt-in as `Video.WrapFix` (M-29, not default; the naive one-liner activates never-run code and boot-crashes `-level_09`, so it needs the hoist-out-of-vertex-loop rework plus a per-level visual check). | `gfx_pc.cpp`; GRAPHICS-BACKLOG B1; §F D74 |
...
3. Character models show obvious texture artifacts during the credits roll.
...
> (`Video.WrapFix`, opt-in). RC1 residual is the 180-degree-rotated front-end photo,
> the D75 family, still open.

| # | Cause | Location | Explains | Fix class |
|---|-------|----------|----------|-----------|
| RC1 | `G_SETTEX` (0xc0) is a no-op in fast3d | `port/fast3d/gfx_pc.cpp:2443` (`case G_NOOP`) | #3 character/model artifacts | Not the blocker: `texLoadFromGdl` already expands it on PC (§6b); residual is the D75 model transform |
| RC2 | Mip-chain contamination: base + all LODs uploaded as one GL image | `src/game/tex.c:330` (`texGetDepthAndSize` sums all LODs) via `gfx_dp_load_block`/`import_texture_*` | #1, #2 noise bands, wrong scale, minification garbage | Fixed, clip LOD block to base-tile height; `Video.FixMipTextures` default on |
...
- Worked example `CheadbrosnanZ`: 47 G_SETTEX commands; texture_ids (1624-1626 = 32x64,
  1607 = 64x32, 1608 = 1x1 null) exactly match the model's `Textures[]` table; the GDL
...
- Our fast3d hits `case G_NOOP: break;`, so every mesh part renders with stale tile state,
  producing wrong/shifted textures on characters. This is why the credits roll (many models,
  many texture switches) shows obvious artifacts.
...
- `texGetDepthAndSize()` (`src/game/tex.c:330`) sums all LOD levels, so one
  ...
  a 65x65 floor texture becomes about 65x134, with rows below the base being raw mip bytes.
- GL then wraps at the uploaded size, and `glGenerateMipmap` plus the never-disabled
...
- About 75% of Depot's textures carry full mip chains (lod 6-7), so this is worst there.
- N64 per-LOD tile selection is already emulated by the D107 rule (always base tile,
  offset 0), so GL mips are pure poison in this port; disable them.
...
- Depot's large room surfaces are non-power-of-two: 65x65, 56x56, 96x48, 128x32
  (I4/IA4, non-zlib). At 65 wide, N64 period 128 vs GL period 65 squashes the pattern
  2x horizontally; with RC2 the vertical period is about 2x too long and half of it is noise.
- The D74 pre-wrap UV block never runs: the guard tests `cms & G_TX_WRAP` but
...
alpha to the wrong bit, so do not do it. If a hue problem is ever confirmed
...
- Texpool exhaustion / D85 placeholder path: a real Depot load (env-gated log) shows
  342 unique texture loads, about 326 KB compressed into the 450 KB stage pool, and zero
...
full 7-level mip chains and non-PoT dimensions (texnums including 85/86, 508, 618, 631,
...
industrial grays/browns, e.g. avg rgb 107/107/108, 147/117/112). The perceived blue is
garbage I4 values sampled under minification (the attract camera is far, so the result
is mip-filtered noise), plus Depot's fog table (`src/game/bgfog.c`, LEVELID_DEPOT entry)
tinting distant surfaces cold. RC4 then shifts whatever hue survives.
...
- ROM layout: `g_Textures[N]` is the segment offset; texture N occupies
...
- Blob header byte: `u(1) z(1) lod(6)`. `z=1` selects the zlib path, otherwise per-image
  format(4)/w(8)/h(8)/comp(4) plus Huffman/RLE/lookup payloads.
- Zlib path (`texInflateZlib`, `image.c:169`): MSB-first bitstream, format(8),
  numcolours(8)+1, palette nc x16; per image: width(8), height(8), then a rarezip stream
  of a 2-byte header `0x11 0x72` plus raw deflate (GE's `src/game/zlib.c` is stock
  gzip-1.2.4 inflate). Output rows are stored tight; `texAlignIndices`
...
- `texSwapAltRowBytes` (`image.c:2191`) is applied to every level of every texture on
  ...
  swapped. The post-swap RAM buffer is what gets uploaded, so any offline tool or dump must
  account for this.
- Palette is appended to the buffer big-endian after all images; `G_LOADTLUT` uploads it.
- `u=0` with lod >= 2: mips are generated at load by `texShrinkPaletted` (2x2 palette average).

## 5. Open questions / hypotheses

- H-A (high confidence): Depot blue ground is RC2+RC3+RC4+fog, per §3. Confirm in-game
  after fixes; the before/after capture is the test.
- H-B (high confidence): credits-roll model artifacts are RC1. Confirm by implementing
...
- H-C (unresolved, non-blocking): the offline contact sheet "still looks off" to the
  ...
  32x32 CI8 textures are inherently noisy at native res, small-cell presentation, or a
...
5. Verification hook: env-gated dump of the actual uploaded GL image + tile setup for
   the first N texLoads (about 10 lines in fast3d). One Depot run gives true ground truth and a
...
- RC1 (G_SETTEX) is not a simple `case` add. `src/game/tex.c texLoadFromGdl`
  ...
  load and `modelNodeReplaceGdl`s the node. It is `#ifdef PORT`-ported. So on PC
  ...
  meaning the model GDL pointer / expansion is broken for the front-end wallet
  models, not that an opcode is missing. This is the D75 front-end-model family
...
- RC2 (mip contamination): the naive height clamp was already tried and
  reverted. `gfx_pc.cpp import_texture()` around line 917 has a comment noting a
  prior branch that set height = `line * tile.height` was removed because it
  "dropped mip chains" and "truncated sub-tiled textures" (Rare-logo
  `D_02005FF0` 20x3 tile uploaded as 32x3). So the fix must distinguish "extra
  rows are mip levels" (drop) from "tile is a window into a larger image"
  (keep). The signal is `rdp.tex_lod` (passed to every `import_texture_*` as
  `gen_mipmaps`): when LOD is on and `size_bytes/row > tile_height`, the excess
  is mip data, so clamp height to `((lrt-ult)>>2)+1` from
  `rdp.texture_tile[tile]` and pass `gen_mipmaps=false` (don't let GL build mips
  from the stacked image). When `rdp.tex_lod` is off, keep today's behaviour.
  Verify against the Rare logo (sub-tile case), the Depot floor (mip case), and
  BUNKER1 (regression).
- The alternating-row look specifically may also be `texSwapAltRowBytes`
  ...
  RenderDoc / §6-item-5 dump task, so do that before editing.

Bottom line (M-30): item 3 is not a grind-fix. The highest-leverage next step is
§6-item-5 (env-gated dump of the actual uploaded GL image + tile setup for the
first about 10 texLoads on the main menu and on Depot) to see whether the base image
...
## 6c. B2 Depot ceiling, root-caused and fixed (D161, M-31)

The blue-speckle + radial-ray roof was not RC2/RC3/RC4/filtering. `GE_TEXDUMP`
(PPM dump of every uploaded texture plus a `fmt/siz/palfmt/palidx/pal[0..3]` log
line, both env-gated, in `gfx_pc.cpp`/`gfx_opengl.cpp`) identified the surface as
one 16x16 CI8 texture uploaded with `rdp.palette_fmt == G_TT_NONE`, while every
...
a legitimate N64 idiom meaning "use the raw 8-bit texel as intensity" (roughly
I8). `import_texture()` ignored `palette_fmt` for `fmt==CI` and did a palette
lookup against a stale `rdp.palette`, producing 16x16 of blue/magenta noise; the
"rays" were that noise aliasing on the receding ceiling plane (hence filtering
never helped).

Fix: `gfx_pc.cpp import_texture()`, `fmt_eff = (fmt==CI && palette_fmt==
G_TT_NONE) ? I : fmt`, dispatch on `fmt_eff` (CI4 to I4, CI8 to I8). Narrow (only
touches currently-garbage surfaces). Depot corridor and control-room ceilings now
...
windows): no crashes, 6/6 frames each; Silo is nearly pixel-identical (phash 0-11),
Facility 180-260 is pixel-identical, Depot shows small localized texel changes on
...
and eyeball the corrugated roof / repeating wall panels against default; flip the

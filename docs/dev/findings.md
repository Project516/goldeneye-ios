...
| D197 | **`-level_09` on this fork renders zero frames, and "VI posts" was hiding it.** The boss thread blocks forever in `osRecvMesg` at `src/boss.c:191`, inside the `MAXCONTROLLERS` loop that does `osSetTimer(&bosstimer, 100000us, 0, &bossmq, &bossmsg)` then `osRecvMesg(&bossmq, ..., OS_MESG_BLOCK)`. The log shows exactly 4 `timer set` / `timer fire` pairs (MAXCONTROLLERS = 4) and then silence, so the loop's last `osRecvMesg` never wakes even though its timer fired. gdb (attach after 20 s) confirms: thread 4 = `bossEntry` -> `bossInitMainthreadData` -> `osRecvMesg(mq=0x3207fed70)`, and the heartbeat dump reads that same queue as `validCount=539233360 msgCount=0` -- the struct is garbage, though `osCreateMesgQueue(&bossmq, &bossmsg, 1)` passed 1. `0x3207fed70` is a boss-thread stack address (window base + `0x2007ed70`, inside `PORT_STACK_REGION`). Suspect a lost or dropped timer post, or the port's `g_pq` queue registry holding a stale entry for a reused stack address. **Not the memory rebase per se** and not a regression from the libc++ header work: identical before and after. The front end is unaffected (1171 frames in 40 s). **Why it went unnoticed:** the port logs `D51 vi post #N`, which is timer-driven and keeps counting whether or not the game ever submits a display list, so a hung level still produces thousands of VI posts. Two verification passes read that as success. Fixed the metric, not just the reading: `portFramesRendered()` (`port/src/libultra.c`) is now logged at quit, so a headless run is judged on frames. | OPEN, root-cause narrowed to the boss.c:191 timer/recv handshake |

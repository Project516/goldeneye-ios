...
| D197 | **`-level_09` rendered nothing: a truncated pointer defeated the mempool resize, so BUNKER1 exhausted MEMPOOL_STAGE and spun in the decomp's deliberate OOM loop.** `fileIndexLoadToBank` (`ob.c:257-281`) deliberately allocates the *entire* remaining bank so `decompressdata` has room, then calls `mempAddEntryOfSizeToBank` to shrink the allocation back to the real decompressed size. That resize only runs `if (allocation == pool->prevpos)`, i.e. "is this still the last allocation". `sub_GAME_7F0762E0` (`objecthandler_2.c:102`) reached it through `fileSetSize` passing `(u8 *)filedata`, where `s32 filedata = (s32)objheader->Switches` -- a **truncated host pointer**. It never equals the pool's real `prevpos`, so the resize silently returned `MEMP_ADD_ENTRY_NOT_LAST_ALLOCATION`, MEMPOOL_STAGE kept a 6.2 MB whole-bank allocation, and the next 48-byte request (a monitor model in `modelmgrInstantiateModel`) hit `while (1);` at `memp.c:229`. Measured: STAGE `pos == end` at `0x70798000`, `prevpos` far below at `0x701e1148`; PERMANENT also full, so the fallback failed too. **Fixed:** pass `objheader->Switches` itself under `#ifdef PORT` (the size argument is a length and is untouched). Raising the port's DRAM ceiling first (`n64stubs.c` `tlbmanageGetTlbAllocatedBlock` `0x70700000` -> `0x707F0000`, all still inside the committed 8 MB) was kept, but it changed nothing on its own: the leak scales with whatever is left, which is the tell for a "take the rest of the bank" allocation that is never given back. **Then:** the same file's `intro_record` walk (`bondview_r.c`, 10 sites of `(s32)intro_record + sizeof(...)`) crashed next, converted to `PORT_PTRADD`, which is now shared from `port/include/n64mem.h` rather than redefined per file. **Still open:** `-level_09` now loads and reaches the render path, crashing in `matrix_4x4_set_lookat` (`matrixmath.c:634`) on a bad `matrix` from `bondviewUpdateCameraMatrices` (`bondview2.c:8217`) -- same family, one level up. **Verification note:** this hid behind `D51 vi post #N`, which is timer-driven and counts whether or not a display list is ever submitted, so a hung level still logs thousands of posts. Two passes were reported clean on that signal. `portFramesRendered()` is now logged at quit; judge runs on frames. | STAGE OOM FIXED (`src/game/objecthandler_2.c`, `src/game/bondview_r.c`, `port/src/n64stubs.c`, `#ifdef PORT`); render-path crash OPEN |

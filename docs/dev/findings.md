...
| D197 | **`-level_09` rendered nothing; fixed, and it turned out to be twelve separate truncation sites, not one bug.** Started as a hang: `fileIndexLoadToBank` (`ob.c:257-281`) deliberately allocates the *entire* remaining bank so `decompressdata` has room, then shrinks it with `mempAddEntryOfSizeToBank`, which only acts `if (allocation == pool->prevpos)`. `sub_GAME_7F0762E0` (`objecthandler_2.c:102`) reached it via `fileSetSize` passing `(u8 *)filedata` where `s32 filedata = (s32)objheader->Switches`, a truncated host pointer, so the shrink silently returned `NOT_LAST_ALLOCATION`, MEMPOOL_STAGE kept a 6.2 MB scratch allocation, and the next 48-byte request hit the decomp's OOM `while (1);` at `memp.c:229`. **Diagnostic tell:** raising the DRAM ceiling by a megabyte changed nothing. A leak that grows to fill whatever you give it is a never-returned whole-bank allocation, not a small pool. Then, in order, each fix exposing the next: the `intro_record` walk (`bondview_r.c`, 10 sites) -> the four s32 player matrix fields 0x5C/0x60/0x64/0x68 (`bondview2.c`) -> the D88 intro-camera `prev` and `lang_ptr` reads -> `if_actor_able_set_on_path`'s `s32 pathid` parameter, which is an address -> the flying-particle buffer's byte-matching `(u32)` cast (`explosion.c`) -> `setFontTables`' two s32 parameters and the `copy_1stfonttable`/`copy_2ndfonttable` globals -> `(s32) &view_left` and siblings into `s32 *` parameters (`bondview2.c:10017`) -> `helddst`, an N64 address into the weapon buffer -> the `ANIMREC` idiom in `chraction.c` and `bondview2.c` -> `(u32)g_IntroSwirl + (u32)base` where `base` had been reused as a byte offset -> `imageoffset + globalbank_rdram_offset` in `gunfire.c`, an N64 address into the global image bank. **Result: `-level_09`, `-level_20` and `-level_27` all load and render crash-free for 40 s.** **Structural changes, not just fixes:** `PORT_PTRADD` and a new `N64_TO_HOST_OR_NULL` live in `port/include/n64mem.h`; `ANIMREC` lives in `initanitable.h` next to the table it indexes, replacing three identical local copies; `n64mem.h` no longer includes `<stddef.h>`/`<stdint.h>`, because including it ahead of `<ultra64.h>` changed something layout-relevant in `bondview2.c` and crashed the front end in `bgApplyDynamicCCRMLUT`, nowhere near the edit. **Verification lesson:** this hid behind `D51 vi post #N`, which is timer-driven and counts whether or not a display list is submitted, so a hung level logs thousands of posts. Two passes were reported clean on that signal. `portFramesRendered()` is now logged at quit; judge runs on frames, and ignore a crash logged *after* the quit line (teardown racing the texture cache). | FIXED across `src/game/{objecthandler_2,bondview_r,bondview2,chraction,chrai,explosion,gunfire,bondview}.c`, `src/game/{chraction,bondview,bondview_internal,initanitable}.h`, `port/src/n64stubs.c`, `port/include/n64mem.h`, all `#ifdef PORT` |
| D198 | **`-level_40` CITADEL hanging is the ROM's own behavior, not a port defect. The solo sweep is 21 / 21, not 21 / 22.** All three game threads sampled under gdb during the hang: `shedThread` and `audioThread` are both parked in `osRecvMesg`, which is idle and correct, and the boss thread is spinning in `langGetLangBankIndexFromStagenum` (`language.c:247`) reached from `proplvreset2` (`prop.c:1274`) <- `lvlStageLoad(stage=40)`. That function is a switch over 27 level ids whose `default:` is the decomp's deliberate `while(1){}` for "unknown text bank", and **`LEVELID_CITADEL` has no case**. It has none because Citadel is the unfinished multiplayer-only arena: it appears in `bgfog.c` only with `ENVIRONMENTDATA_PLAYERS_2/3/4` variants, and it has no briefing or objective text to load. Loading it as a solo mission hangs on hardware for the same reason. **Per non-negotiable 2 this must not be "fixed"** — adding a case would be a game-logic change to make an unsupported path work. `-level_40` is dropped from `tools_pc/sweep.sh`'s default list instead. **Method note:** the POSIX `crashDumpThreads` can only backtrace the calling (watchdog) thread, so the heartbeat report shows the watchdog and tells you nothing about who is stuck. Attach gdb and sample all threads. Filter by stack content, not thread name: Mesa's llvmpipe pool threads inherit the process name and drown the four that matter. | NOT A BUG — original behavior, documented, level removed from the sweep list |
| D199 | **The asset sidecars are not reproducible across machines, and that is fine. Do not chase the diff.** `data/pccg-<region>/pccg.bin` and `data/pcmodels-<region>/pcmodels.bin` built on the macOS CI runner differ from the same commit built on Fedora in every converted record: `pcmodels.bin` was 1315680 vs 1309408 bytes, `sum round8(C_pc)` 1313768 vs 1307040, and `max C` went the *other* way (0x430e vs 0x4324). Inputs are identical -- same ROM sha1 `abe01e4a`, same 512 of 512 files converted, same 512 committed `modelFileHeader.inc.c`, and the uncompressed `bg/*.seg` rows in `pccg-*/manifest.csv` match byte for byte. **Cause:** `d43_emit.py:677` recompresses each record with `zlib.compressobj(6, DEFLATED, -15)`, and Fedora's Python links **zlib-ng** (`zlib.ZLIB_RUNTIME_VERSION == "1.3.1.zlib-ng"`) while the macOS runner links stock zlib. Two different deflate implementations at the same level emit different-length streams. Both are valid raw deflate, the port decompresses either, and `manifest.csv` carries the real sizes, so the artifact is correct either way. **Consequence worth knowing:** d43's headroom asserts (`P_est` vs the fixed N64 buffer sizes) are evaluated against whichever compressor ran, so it is the **CI** run's "ALL CHECKS PASSED" that certifies a shipped sidecar, not a local one. **How this was found:** by diffing the ios-5 release sidecars against the local `data/` and assuming a bug. Reproducing in a clean worktree at the CI commit, then again after a full `extract_baserom.u.sh`, gave the local number both times, which ruled out the tree and pointed at the toolchain. | NOT A BUG — expected, documented |
| D200 | **iOS app launches to a permanent black screen: alive, no crash report, and no `ge007.log`. Cause still open; four hypotheses are ruled out, all by reading the shipped artifact.** Verified against the `ios-5` IPA itself, not the source: **(a) file sharing is configured** -- the built `Info.plist` has `UIFileSharingEnabled` and `LSSupportsOpeningDocumentsInPlace` both true and `CFBundleExecutable = GoldenEye`, so a non-empty Documents would show in Files. **(b) the SDL iOS entry point is wired** -- the Mach-O symbol table has `_SDL_main` defined at `0x100011d7c` (our text) and `_main` at `0x10017f6dc` (SDL's region) plus `_SDL_UIKitRunApp`, i.e. libSDL2main.a's `main` is linked and calls ours. The rename arrives for free: `port/include/video.h:13` includes `<SDL.h>`, which includes `SDL_main.h`, which does `#define main SDL_main`. **(c) `PLATFORM_IOS` is defined in the build** -- the strings `ge007.log`, `Documents` and `no ROM found. Tried` are all in the binary, so the Documents path resolution and the log tee are compiled in, not guarded out. **(d) it is not the "run offscreen" work** -- those commits only touched the Linux Xvfb test script, nothing compiled into the app. **My wrong turn, worth the space:** I diagnosed (b) as the cause and committed a `#define main SDL_main` for it *before* looking at the artifact. One symbol-table dump disproved it. The IPA is downloadable and its plist, symbol table and string table each answer a whole class of hypothesis in one command -- read it first. **Next instrument:** `sysShowMessage()` (SDL_ShowSimpleMessageBox, iOS-only, implemented in video.c because it is the only port file that already includes SDL) now fires on the ROM-missing path after a `videoInit()` that depends on nothing but config, so the device can report whether `main` ran at all and where it looked. | OPEN — cause unknown, four hypotheses eliminated, on-screen reporting added to get evidence off the device |

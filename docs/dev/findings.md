...
| D200 | **The first iOS IPA was a permanent black screen with no crash and no log: `port/src/main.c` defined a plain `main()`, so SDL's iOS entry point was never linked.** On iOS the real `main()` is in `libSDL2main.a` and does `SDL_UIKitRunApp(argc, argv, SDL_main)`, which is the call that creates the `UIApplication` and its run loop. `main.c` includes no SDL header at all, so its own `main` satisfied the `_main` symbol and the linker never pulled SDL2main's object. No `UIApplication` means no run loop and no window, so iOS just leaves the launch screen up. `UILaunchScreen` is an empty dict, which renders as solid black -- hence black, alive, not crashing. Fixed with `#define main SDL_main` under `PLATFORM_IOS`; `SDL_main.h` does this with a macro but drags `SDL_stdinc.h` through the decomp header shims, and this file deliberately includes no SDL. With `main` renamed, `_main` is undefined and the linker resolves it out of libSDL2main.a, which is what we want. **Diagnostic note:** the absence of a `ge007.log` in the app's Documents folder was the tell, together with the Info.plist inside the shipped IPA being verifiably correct (`UIFileSharingEnabled` and `LSSupportsOpeningDocumentsInPlace` both true), which ruled out file sharing. Unpack the IPA and read its built plist rather than reasoning about the template. | FIXED (`port/src/main.c`, `#if defined(PLATFORM_IOS)`) |

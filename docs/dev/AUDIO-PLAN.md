# Audio plan, Phase 3 (libaudio to SDL)
...
5. [Architecture options](#5-architecture-options-evaluate-all-recommendation-marked), options A/B/C plus sub-decisions A1-A3
6. [DMEM layout](#6-dmem-layout-ido-from-srclibultraaudiosynthinternalsh5771)
7. [Shims and ABI fixes required](#7-shims-and-abi-fixes-required), including [7.1 K0_TO_PHYS bug](#71-bug-found-k0_to_phys-is-unshimmed-and-corrupts-pointers-on-pc)
...
- SDL audio device and queue plumbing (`port/src/audio.c`, AI shims in `libultra.c`).
...
Working already:
- The whole CPU side of libaudio runs. `src/libultra/audio/*` plus
  ...
  compiled, a Phase 2 decision), calls `alAudioFrame()`, which fills a 3000-word Acmd
  list and posts an `M_AUDTASK` to the scheduler. Bank trees and cseq fixups (D54, D36/D37)
...
- C7 / D127 (`src/snd.c:859`): skips SFX whose resolved `ALSound*` pointer is out of
  range, added because the converted bank tree (`romdata.c afFixupInst`) yields fewer or
...
resampler, envmixer, then FX: GE's custom chorus/reverb, aux bus, main bus, save)
...
Opcode inventory (grep-verified across `src/libultra/audio/*.c` plus
`src/libultrare/audio/*.c`, 15 ops):

| Op | Emitted by | PD reference code? |
|---|---|---|
| `aSetBuffer` (x20) | load, mainbus, resample, save, reverb | yes (`a*Impl` pattern; different arg ABI) |
| `aMix` (x7) | mainbus, reverb | yes, `aMixImpl` |
| `aLoadBuffer` (x7) | load, reverb | yes, `aLoadBufferImpl` |
| `aClearBuffer` (x7) | auxbus, mainbus, load, reverb | yes, `aClearBufferImpl` |
| `aSetVolume` (x5) | env.c (Rare envelope) | yes, `aSetVolumeImpl` |
| `aSaveBuffer` (x4) | save, reverb | yes, `aSaveBufferImpl` |
| `aDMEMMove` (x4) | load, resample, reverb | yes, `aDMEMMoveImpl` |
| `aResample` (x2) | resample.c, reverb.c | yes, `aResampleImpl` (SSE4.1/NEON/scalar) |
| `aLoadADPCM` (x2) | load.c, reverb.c | yes, `aLoadADPCMImpl` |
...
PD reference means the Perfect Dark port `port/src/mixer.c` plus
`port/include/mixer.h`. Reusable: the DSP math (ADPCM 8-coefficient loop filter,
resample table plus interpolation, envelope ramping, clamps). Not reusable as-is:
signatures and argument conventions (naudio ABI differs from IDO ABI, §5.3).
...
3. PD replaced the scheduler (`pdsched.c`) and drives audio from retrace:
   `schedAudioFrame()` calls `amgrFrame(); audioEndFrame();` (times `diffframe60`). Same
   thread for build and flush, so there is no locking in their SDL handoff.
4. SDL device layer: queue-based, exactly what our scaffold already copied.
   `osAiGetLength` returns queued bytes (`audioGetBytesBuffered()`),
   `osAiSetNextBuffer` stashes the pointer, `audioEndFrame` calls `SDL_QueueAudio` with a
   `queueLimit` backpressure drop. Device opened at 22020 Hz (their TODO admits this
   "might cause trouble", a silent 9-cent detune vs their 22050 synth rate).
5. DMEM: about 3 KB static buffer, naudio-specific offsets (main L/R at 0x4E0/0x650,
   aux at 0x7C0/0x930), different from IDO's layout (§6).
...
- IDO's implicit "current buffer": e.g. `aADPCMdec(pkt, f, s)` carries only flags plus
...
- Pros: zero game/header/build changes; treats the stream as data exactly like fast3d
  does with GBI (project-consistent); natural logging hook (`GE_DAUDIO=1` dump of op plus
...
- Cons: must write the word decoder (about 50 lines, trivial, op is bits 24-31 of w0 per
...
- Cons: shadows a core PR header for every TU that includes it (only libaudio uses these
  macros on PC, so blast radius is small but nonzero, and include-path ordering in
  `CMakeLists.txt` needs verifying); stream becomes invisible at runtime (no logging hook,
  instrumentation would have to go inside each Impl); the shim must exactly mirror IDO
  arg order, which differs from PD's naudio redirects line-by-line.
...
RSP core plus DMEM and an interrupt model for one component whose entire job is the 15 ops we
...
- Deferred (PD-style): stash in `osAiSetNextBuffer`, flush from a frame-end hook.
  In GE this crosses threads (audio thread sets; scheduler/gfx path would flush), so it
  needs a mutex or has to accept PD's unlocked pattern, and needs a new `audioEndFrame()`
  call site that doesn't exist today. Only worth it if immediate queueing shows a problem.
...
### Sub-decision A3, timing and thread priorities (recommended: inline plus host priorities, no worker thread)
...
normal, a roughly 10-line fix in `libultra.c`. A dedicated audio worker thread consuming
...
and `load.c:479` (`aSetLoop(ptr++, K0_TO_PHYS(f->lstate))`), so the mixer would receive
...
- `osAiSetFrequency(hz)` should return `hz` (already does; 22050 flows into
  `alconf->outputRate` and the frame-size math, which is correct).
- `osAiSetNextBuffer(buf, size_bytes)` should queue to SDL per A1.
- `osAiGetLength()` should return bytes currently queued (`SDL_GetQueuedAudioSize`); the
  game does `>> 2` for frames (`audi.c:527`). PD returns bytes too. This closes the
...
- `port/src/mixer.c`: rewrite as the op implementations plus (for Option A) the decoder
  and current-buffer state machine. `mixerInit/Destroy` reset DMEM plus op state.
- Threading note: mixer runs on the scheduler thread (`sched.c __scMain` calling
  `osSpTaskStartGo` inline); with A1 there is exactly one shared object (the SDL queue,
  internally locked), so no port-side locks are needed.

## 8. Phase 0, recon (do first; informs everything)

1. Disassemble `bin/aspboot.text.bin` (plus `.data.bin`; markers in `src/aspboot.s`),
   ...
   - exact `aSetBuffer` current-buffer semantics (which subsequent ops consume it, and
     how),
   - `aADPCMdec` state layout (what the pointer in w1 points at, IDO `ADPCM_STATE` plus
     data addressing),
...
2. Acmd stream logger (`GE_DAUDIO=1`, in the Option-A decoder or a temporary probe
   in the `M_AUDTASK` branch): dump op plus decoded args per frame for the first N frames.
...
- Q2, Option A vs B (§5): A is recommended (no header shadowing, stream logging,
  ...
  (§6 layout, all op bodies); only the roughly 50-line front end (word decoder vs macro
...
- Phase 0, recon (§8): microcode disassembly notes plus Acmd log artifacts committed
  to `docs/` (or `tools_pc/`). Exit: current-buffer semantics and pole filter documented;
  op inventory confirmed at runtime.
- Phase 1, plumbing. SDL device opens at 22050 stereo s16 (A2); A1 queue path live;
...
- Phase 2, core ops. Implement `aSetBuffer/aLoadBuffer/aClearBuffer/aDMEMMove/
  ...
  aSaveBuffer` (plus `aSegment` no-op) in `mixer.c`, reusing PD's DSP math where the ABI
...
- Phase 3, FX. `aPoleFilter` plus validate the full `reverb.c` chorus path against
  the disassembly. Milestone: music matches N64 character (chorus/reverb present, no
  pumping); close D77.
- Phase 4, polish and bookkeeping. Resolve Q1; remove/adjust temporary probes and
  ...
  record findings in `docs/internals.md` §F/§H (next Dxx after D127) plus update the
...
1. Boot to title: menu music audible within a couple of seconds; no click at start.
...
4. Run 2-3 levels including one with heavy SFX overlap; watch `SDL_GetQueuedAudioSize`
...
6. `Audio.Enabled=0` should be fully silent, zero audio CPU cost, nothing else affected.

## 12. Risks / unknowns

| Risk | Mitigation |
|---|---|
| IDO current-buffer semantics misread, leading to subtle wrong output (no crash) | Phase 0 disassembly is the arbiter; Acmd logger plus by-ear checks per phase |
| `aPoleFilter` has no PD code | Disassembly (Q3); it's one small IIR, bounded effort |
| ADPCM state layout differs from assumption, risking garbage/scratchy samples | Phase 0 documents the struct from `load.c` usage plus microcode; scalar fallback path makes debugging easy |
...
|---|---|
| `src/audi.c` | audio thread, frame-size feedback loop, task construction (`OUTPUT_RATE` at 22) |
| `src/sched.c:439,465,485` | real scheduler's M_AUDTASK path, `osSpTaskStartGo` interception point |
| `include/PR/abi.h:260-407` | IDO Acmd word formats (the decoder's spec) plus A_* opcodes |
| `src/libultra/audio/synthInternals.h:57-71` | DMEM layout constants |
| `src/libultrare/audio/reverb.c`, `env.c`, `drvrNew.c` | GE custom FX (`alFxNew`), pole filter plus chorus, envelope ops |
| `bin/aspboot.text.bin` / `.data.bin` (plus `src/aspboot.s`) | ground-truth ASP microcode to disassemble |
| `pd_port/port/src/mixer.c`, `port/include/mixer.h` | DSP math reference plus the Option-B pattern |
...
| `port/src/dram.c`, `port/shim/PR/R4300.h` | address model (V1/V2 views) plus the K0_TO_PHYS gap |

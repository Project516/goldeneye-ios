...
32-bit N64 game code, unmodified, on a little-endian 64-bit host. It's terse
by design. Each entry compresses a full investigation down to a symptom, a
fix, and a grep heuristic for finding siblings. Each entry cites a `Dxx`
label; the full evidence and fix for that instance live in
[`dev/findings.md`](dev/findings.md) under the same label. Skim the section
headers, then read the classes relevant to the task at hand.

If you're debugging a crash in this port, read this first. The odds are
good you're looking at one of these.
...
- [D2. The HUD/model "X-mirror" (D114/D116). RESOLVED: it was an upside-down capture](#d2-the-hudmodel-x-mirror-d114d116-resolved-it-was-an-upside-down-capture)
- [D3. GCC/mingw makes an all-non-negative `enum` UNSIGNED](#d3-gccmingw-makes-an-all-non-negative-enum-unsigned)
- [D4. N64 "interrupts off" is not free on PC: it must be a real lock](#d4-n64-interrupts-off-is-not-free-on-pc-it-must-be-a-real-lock)
...
- **Diagnosing a "wrong texture" needs the *raw importer input*, not the
  ...
  image has a sharp minimum at its true pitch (approx. 0.2-0.7 on a 0-15 nibble
...
- **D152 addendum (M-31 static audit).** Every `osSetIntMask(OS_IM_NONE)` in
  ...
  `alEvtqPostEvent`, whose lone early `return` at `event.c:86` **does** restore

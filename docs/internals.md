# GoldenEye 007 PC port: architecture and plan
...
* **Saves** are **EEPROM-based** (controller EEPROM via `osEeprom*`), not
  ...
  file-backed EEPROM plus no-op PFS/motor shims. That's simpler than a full Memory
  Pak emulation.
...
to PC-native layout *at load time*. That's a systematic solution to the problem class
...
- `filemodel.c` (1,114): the direct **D43** analogue, same `PROMOTE` idiom,
  **same vma 0x5000000**. NOT a drop-in copy (GE's `ModelRoData` union + opcode
  set differs, PD has stargunfire/headspot/gundl node types), but the node-walk
...
* **Engine layout** (same family as PD):
  ...
  * `src/init.c`: `init()` -> `mainproc()` -> `bossEntry()` (game entry).
    **Compiled for the PC** (provides `mainproc()`); its N64-only `init()` is
    stubbed (see A3).
* **Custom RSP microcode:** `rsp/graphics/gmain.s` (1545 lines) is a modified
...
2. **`G_SETTEX`** (`0xc0`), meaning "use texture from bank": selects a texture by
...
* **Controllers:** `src/joy.c` uses `osContInit` / `osContStartReadData`.
  ...
  structs. PD implements 1964GEPD-style and Xbox-style bindings, which is a good

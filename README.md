...
An in-progress **iOS port** of GoldenEye 007 (N64), forked from
...
Stage 0 is complete. Nothing is playable on iOS yet.

iOS cannot map any address below `0x100000000`, which is where the PC port
places the cart image and both DRAM views. Measured on device: all three fail
with `KERN_INVALID_ADDRESS`. The replacement, a single 4 GiB-aligned window
with `vm_remap` aliasing for the KSEG0 mirror, is verified working on the same
device, including thread stacks. Stage 1 is rebasing the port onto it.

Full findings and plan: `docs/ios/plan.html`. Working notes: `docs/ios/AGENTS.md`.

## You need your own ROM

No ROM data is included here, in the build, or in any artifact. Supported:
GoldenEye 007 (USA), sha1 `abe01e4aeb033b6c0836819f549c791b26cfde83`.

## Building the iOS probe
